  ...
  module CustomerDetails
    module ResponseEntity
      class RetCustDtls < SerializableObject
        attr_accessor :phone_email_infos, :ret_cust_addr_infos, :acct_name, :annual_revenue, :assistant, :available_cr_limit, :blacklist_notes, 
        ...
        def initialize(options={})
          options ||= {}
          ret_cust_addr_infos = options.delete(:ret_cust_addr_info)
          if ret_cust_addr_infos 
            if ret_cust_addr_infos.is_a?(Array)
              @ret_cust_addr_infos = ret_cust_addr_infos.map { |result| RetCustAddrInfo.new(result) }
            else
              @ret_cust_addr_infos = [RetCustAddrInfo.new(ret_cust_addr_infos)]
            end
          end
          # @ret_cust_addr_info = RetCustAddrInfo.new(options.delete(:ret_cust_addr_info))
          phone_email_infos = options.delete(:phone_email_info)
