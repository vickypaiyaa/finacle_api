    ...
    module ResponseEntity
      class LoanAcctInqRs < SerializableObject
        ...
        def initialize(options={})
          ...
          if multi_src_instruction_recs 
            if multi_src_instruction_recs.is_a?(Array)
              @multi_src_instruction_recs = multi_src_instruction_recs.map { |result| MultiSrcInstructionRec.new(result) }
            else
              @multi_src_instruction_recs = [MultiSrcInstructionRec.new(multi_src_instruction_recs)]
            end
          end

          rel_party_recs = options.delete(:rel_party_rec)
          if rel_party_recs 
            if rel_party_recs.is_a?(Array)
              @rel_party_recs = rel_party_recs.map { |result| RelPartyRec.new(result) }
            else
              @rel_party_recs = [RelPartyRec.new(rel_party_recs)]
            end
          end
